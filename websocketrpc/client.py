...
class Client(object):
    ...
    def ws_connection_cb(self, conn):
        ...
        # Overwrite method. Dirty, but AFAIK inheritance is not possible here.
        self.ws_connection.on_message = self.on_message
        self.keepalive = self.ioloop.add_timeout(
            datetime.timedelta(seconds=self.KEEPALIVE_TIMEOUT),
            self.do_keep_alive)

    def call(self, func_str, args=None, kwargs=None,
             on_reply=None, on_error=None):
        '''
         Caller calls func on remote
         '''
        json_request = self.protocol.create_request(
            func_str,
            args=args,
            kwargs=kwargs)
